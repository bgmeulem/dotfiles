...
Requirements:
  pip install pychromecast RangeHTTPServer
  and have `catt` available on your PATH.
...
VIDEO_FILE = sys.argv[1] if len(sys.argv) > 1 else "movie.mp4"
MPV_SOCKET = "/tmp/mpv-audio.sock"
SYNC_INTERVAL = 0.3
...
def create_muted_video(src_path):
    ...
    tmp_muted = f"/tmp/{os.path.basename(src_path)}"
    print("Creating muted video for Chromecast...")
    r = subprocess.run(
