...
if [ "$previous_power_state" != "$current_power_state" ]; then
    ...
    printf -v ROUNDED_BC "%03d" "$ROUNDED_BC"
    case "$current_power_state" in
        "connected")
            ...
            if [ -f "$NOTIFICATION_ID_FILE" ]; then
                ...
                notify-send --replace-id="$NOTIFICATION_ID" "Power Connected" "Battery charging (${BATTERY_CAPACITY}%)" -i "battery-${ROUNDED_BC}-charging" -t "$NOTIF_TIMEOUT_MS";
                rm -f "$NOTIFICATION_ID_FILE"
            else
                # Show general power connected notification
                notify-send "Power Connected" "Battery charging (${BATTERY_CAPACITY}%)" -i "battery-${ROUNDED_BC}-charging" -t "$NOTIF_TIMEOUT_MS"
            fi
            ;;
        "disconnected")
            # Power just disconnected
            notify-send "Power Disconnected" "Running on battery (${BATTERY_CAPACITY}%)" -i "battery-${ROUNDED_BC}" -t "$NOTIF_TIMEOUT_MS"
            ;;
    esac
